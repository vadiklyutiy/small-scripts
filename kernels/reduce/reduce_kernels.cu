...
// Kernel 1: 32 threads, atomic (optimized pattern: 64 elements per thread)
...
// Kernel 3: 64 threads, atomic (optimized pattern: 32 elements per thread)
__global__ void reduce_mean_64_atomic(float* input, float* output, int width) {
    ...
    // Each thread loads 32 elements (2048/64 = 32)
    float rv = 0.0f;
    for (int i = 0; i < 32; i++) {
        int idx = tid + i * 64;
        if (idx < width) {
    ...
    // Write final result
    if (tid == 0) {
        output[row] = shared_sum / width;
    }
}

// Kernel 4: 64 threads, separate (optimized pattern: 32 elements per thread)
__global__ void reduce_mean_64_separate(float* input, float* output, int width) {
    ...
    if (tid == 0) {
        ...
        output[row] = total_sum / width;
    }
}

// Kernel 5: 128 threads, atomic (optimized pattern: 16 elements per thread)
__global__ void reduce_mean_128_atomic(float* input, float* output, int width) {
    ...
    // Each thread loads 16 elements (2048/128 = 16)
    float rv = 0.0f;
    for (int i = 0; i < 16; i++) {
        int idx = tid + i * 128;
        if (idx < width) {
...
// Kernel 6: 128 threads, separate (optimized pattern: 16 elements per thread)
...
// Kernel 7: 256 threads, atomic (optimized pattern: 8 elements per thread)
__global__ void reduce_mean_256_atomic(float* input, float* output, int width) {
    ...
    // Each thread loads 8 elements (2048/256 = 8)
    float rv = 0.0f;
    for (int i = 0; i < 8; i++) {
        int idx = tid + i * 256;
        if (idx < width) {
...
// Kernel 8: 256 threads, separate (optimized pattern: 8 elements per thread)
...
// Kernel 9: 512 threads, atomic (optimized pattern: 4 elements per thread)
...
// Kernel 10: 512 threads, separate (optimized pattern: 4 elements per thread)
...
// Kernel 11: 1024 threads, atomic (optimized pattern: 2 elements per thread)
__global__ void reduce_mean_1024_atomic(float* input, float* output, int width) {
    ...
    // Each thread loads 2 elements (2048/1024 = 2)
    float rv = 0.0f;
    
    // Load first element
    if (tid < width) {
        rv += input[row * width + tid];
    }
    
    // Load second element (offset by 1024)
    if (tid + 1024 < width) {
        rv += input[row * width + tid + 1024];
    }
...
// Kernel 12: 1024 threads, separate (optimized pattern: 2 elements per thread)
